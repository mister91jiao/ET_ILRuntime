...
namespace ET
{
    ...
    public class ResourcesComponent: Entity
    {
        ...
        public async ETTask UnloadBundles(List<string> bundleList, bool unload = true)
        {
            ...
            {
                CoroutineLock coroutineLock = await CoroutineLockComponent.Instance.Wait(CoroutineLockType.Resources, bundle.GetHashCode());
                {
                    if (++i % 5 == 0)
                    {
        ...
        public async ETTask LoadBundleAsync(string assetBundleName)
        {
            ...
            using (var abInfos = MonoListComponent<ABInfo>.Create())
            {
                ...
                using (MonoListComponent<ETTask> tasks = MonoListComponent<ETTask>.Create())
                {
                    foreach (string dependency in dependencies)
                    {
        ...
        {
            CoroutineLock coroutineLock = await CoroutineLockComponent.Instance.Wait(CoroutineLockType.Resources, abInfo.Name.GetHashCode());
            {
